<?php
/**
 * ThemeHungLv functions and definitions
 *
 * @package ThemeHungLv
 */
...
if ( ! function_exists( 'blaskan_setup' ) ) :
	...
	function blaskan_setup() {
		 ...
		 */
		load_theme_textdomain( 'ThemeHungLv', get_template_directory() . '/languages' );
		...
		register_nav_menus( array(
            'menu-1'      => esc_html__( 'Primary', 'ThemeHungLv' ),
            'menu-2'      => esc_html__( 'Footer', 'ThemeHungLv' ),
            'social-menu' => esc_html__( 'Social Menu', 'ThemeHungLv' ),
        ) );
...
function blaskan_widgets_init() {
	register_sidebar( array(
          'name'          => esc_html__( 'Sidebar', 'ThemeHungLv' ),
          'id'            => 'sidebar-1',
          'description'   => esc_html__( 'Add widgets here.', 'ThemeHungLv' ),
	...
	if ( $number == 1 ) {
		...
		register_sidebar( array(
			'name'          => esc_html__( 'Footer Sidebar', 'ThemeHungLv' ),
			'id'            => 'footer-sidebar',
			'description'   => esc_html__( 'Add widgets here.', 'ThemeHungLv' ),
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
		...
		register_sidebars( $number, array(
			'name'          => esc_html__( 'Footer Sidebar %d', 'ThemeHungLv' ),
			'id'            => 'footer-sidebar',
			'description'   => esc_html__( 'Add widgets here.', 'ThemeHungLv' ),
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
...
function blaskan_scripts() {
	...
	wp_enqueue_style( 'ThemeHungLv-fonts', blaskan_fonts_url() );
	wp_enqueue_style( 'bootstrap', get_template_directory_uri() . '/assets/css/bootstrap.min.css' );
	wp_enqueue_style( 'fontawesome', get_template_directory_uri() . '/assets/css/font-awesome.min.css' );
	wp_enqueue_style( 'ThemeHungLv-style', get_stylesheet_uri() );

	wp_enqueue_script( 'imagesloaded' );
	wp_enqueue_script( 'masonry' );
	wp_enqueue_script( 'ThemeHungLv-navigation', get_template_directory_uri() . '/assets/js/navigation.js', array(), '20151215', true );
	wp_enqueue_script( 'ThemeHungLv-skip-link-focus-fix', get_template_directory_uri() . '/assets/js/skip-link-focus-fix.js', array(), '20151215', true );
	wp_enqueue_script( 'ThemeHungLv-scripts', get_template_directory_uri() . '/assets/js/scripts.js', array( 'jquery' ), '20151215', true );
...
function blaskan_excerpt_more( $link ) {
	...
	$link = blaskan_create_read_more_link( sprintf( '<p class="link-more"><a href="%1$s" class="more-link">%2$s <span class="meta-nav">→</span></a></p>',
	                 ...
	                 esc_html__( 'Continue reading', 'ThemeHungLv' )
	));
...
function blaskan_fonts_url() {
	...
	*/
	$droid = _x( 'on', 'Droid Serif font: on or off', 'ThemeHungLv' );

	/* Translators: If there are characters in your language that are not
	* supported by Source Sans Pro, translate this to 'off'. Do not translate
	* into your own language.
	*/
	$source_sans = _x( 'on', 'Source Sans Pro font: on or off', 'ThemeHungLv' );

	/* Translators: If there are characters in your language that are not
	* supported by Work Sans, translate this to 'off'. Do not translate
	* into your own language.
	*/
	$work_sans = _x( 'on', 'Work Sans font: on or off', 'ThemeHungLv' );

	/* Translators: If there are characters in your language that are not
	* supported by Pacifico, translate this to 'off'. Do not translate
	* into your own language.
	*/
	$pacifico = _x( 'on', 'Pacifico font: on or off', 'ThemeHungLv' );
 ...
 */
require get_template_directory() . '/inc/class-ThemeHungLv-theme-plugin-enhancements.php';

/**
 * Load theme widgets.
 */
require get_template_directory() . '/inc/class-ThemeHungLv-author-widget.php';
