...
class Blaskan_Author_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			...
			esc_html__( '[Blaskan] Author Widget', 'ThemeHungLv' ), // Name
			array( 'description' => esc_html__( 'A widget to show author info', 'ThemeHungLv' ), ) // Args
		);
	}
	...
	public function form( $instance ) {
		global $wp_customize;
		$title = ! empty( $instance['title'] ) ? $instance['title'] : esc_html__( 'About Author', 'ThemeHungLv' );
		...
		?>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_attr_e( 'Title:', 'ThemeHungLv' ); ?></label>
			<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>">
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'author' ) ); ?>"><?php esc_attr_e( 'Author:', 'ThemeHungLv' ); ?></label>
			<select id="<?php echo esc_attr( $this->get_field_id( 'author' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'author' ) ); ?>">
				<option value="0"><?php _e( 'Select an user', 'ThemeHungLv' ); ?></option>
				<?php
		...
		<p class="nav-menu-widget-no-menus-message" <?php if ( ! empty( $menus ) ) { echo ' style="display:none" '; } ?>>
			...
			?>
			<?php echo sprintf( __( 'No menus have been created yet. <a href="%s">Create some</a>.', 'ThemeHungLv' ), esc_attr( $url ) ); ?>
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'social_menu' ) ); ?>"><?php esc_attr_e( 'Social Menu:', 'ThemeHungLv' ); ?></label>
			<select id="<?php echo $this->get_field_id( 'social_menu' ); ?>" name="<?php echo $this->get_field_name( 'social_menu' ); ?>">
				<option value="0"><?php _e( '&mdash; Select &mdash;', 'ThemeHungLv' ); ?></option>
