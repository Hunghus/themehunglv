...
class Blaskan_Theme_Plugin_Enhancements {
	...
	function __construct() {
		...
		$this->plugins = array(
			array(
				...
				'message' => sprintf(
					esc_html__( 'The %1$s is necessary to use some of this theme&rsquo;s features, including: ', 'ThemeHungLv' ),
					'<strong>' . esc_html__( 'Jetpack plugin', 'ThemeHungLv' ) . '</strong>' ),
	...
	function get_theme_dependencies() {
		...
		if ( current_theme_supports( 'jetpack-content-options' ) ) :
			$dependencies['content'] = array(
				'name'   => esc_html__( 'Content Option', 'ThemeHungLv' ),
	...
	function get_module_name( $module ) {
		$module_names = array(
			'none'                 => esc_html__( 'no specific module needed', 'ThemeHungLv' ),
			'custom-content-types' => esc_html__( 'Custom Content Types module', 'ThemeHungLv' ),
		);
	...
	function admin_notices() {
		...
		foreach ( $this->plugins as $key => $plugin ) {
			...
			if ( 'to-activate' === $plugin['status'] ) {
				...
				$notice       .= sprintf(
					esc_html__( ' Please activate %1$s. %2$s', 'ThemeHungLv' ),
					esc_html( $plugin['name'] ),
					( $activate_url ) ? '<a href="' . $activate_url . '">' . esc_html__( 'Activate', 'ThemeHungLv' ) . '</a>' : ''
				);
			}
			...
			if ( 'to-install' === $plugin['status'] ) {
				...
				$notice      .= sprintf(
					esc_html__( ' Please install %1$s. %2$s', 'ThemeHungLv' ),
					esc_html( $plugin['name'] ),
					( $install_url ) ? '<a href="' . $install_url . '">' . esc_html__( 'Install', 'ThemeHungLv' ) . '</a>' : ''
				);
			}
		...
		foreach ( $this->unactivated_modules as $module => $features ) :
			...
			$notice .= '<p>';
			$notice .= sprintf(
				esc_html__( 'To use %1$s, please activate the Jetpack plugin&rsquo;s %2$s.', 'ThemeHungLv' ),
...
function dismiss_admin_notice() {
	...
	if ( ! isset( $_POST['blaskan_jetpack_admin_nonce'] ) || ! wp_verify_nonce( $_POST['blaskan_jetpack_admin_nonce'], 'blaskan_jetpack_admin_nonce' ) ) {
		wp_die( esc_html__( 'Your request failed permission check.', 'ThemeHungLv' ) );
	}
	...
	wp_send_json( array(
		              'status'  => 'success',
		              'message' => esc_html__( 'Your request was processed. See ya!', 'ThemeHungLv' )
	              ) );
}
