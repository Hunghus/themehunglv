...
function blaskan_customize_register( $wp_customize ) {
	...
	$wp_customize->add_section( 'blaskan_theme_options', array(
		'title'    => esc_html__( 'Theme Options', 'ThemeHungLv' ),
		'priority' => 33,
	) );
	...
	$wp_customize->add_control( 'blaskan_disable_header_search', array(
		'label'   => esc_html__( 'Hide Header Search', 'ThemeHungLv' ),
	...
	$wp_customize->add_control( 'blaskan_site_layout', array(
		'label'   => esc_html__( 'Site layout', 'ThemeHungLv' ),
		'section' => 'blaskan_theme_options',
		'type'    => 'radio',
		'choices' => array(
			'right-sidebar' => esc_html__( 'Right Sidebar', 'ThemeHungLv' ),
			'left-sidebar'  => esc_html__( 'Left Sidebar', 'ThemeHungLv' ),
			'no-sidebar'    => esc_html__( 'No Sidebar', 'ThemeHungLv' ),
		),
	) );
	...
	$wp_customize->add_control( 'blaskan_footer_column', array(
		'label'   => esc_html__( 'Footer Area Layout', 'ThemeHungLv' ),
		'section' => 'blaskan_theme_options',
		'type'    => 'radio',
		'choices' => array(
			'column-1' => esc_html__( '1 Column', 'ThemeHungLv' ),
			'column-2' => esc_html__( '2 Columns', 'ThemeHungLv' ),
			'column-3' => esc_html__( '3 Columns', 'ThemeHungLv' ),
			'column-4' => esc_html__( '4 Columns', 'ThemeHungLv' ),
		),
	) );
		...
		'render_callback' => function() {
            ...
            if ( $disable_search ) {
            	...
            	$output .= '<div id="search-header-form" class="search"><form role="search" method="get" class="search-form" action="'.esc_url( home_url( '/' ) ).'">';
                    $output .= '<input id="search" type="search" name="s" placeholder="'.esc_html__( 'Search ...', 'ThemeHungLv' ).'">';
